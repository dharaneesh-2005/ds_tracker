...
export interface IStorage {
  ...
  deleteStudentGoal(reg_no: string, goal_id: number): Promise<boolean>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getStudentProgressWithAllProblems(reg_no: string): Promise<(Problem & { status?: string; notes?: string })[]> {
    const allProblems = await db.select().from(problems).orderBy(problems.id);
  ...
  async deleteStudentGoal(reg_no: string, goal_id: number): Promise<boolean> {
    const result = await db
      .delete(student_goals)
      .where(and(
        eq(student_goals.reg_no, reg_no),
        eq(student_goals.id, goal_id)
      ));
