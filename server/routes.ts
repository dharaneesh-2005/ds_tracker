...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put("/api/admin/students/:reg_no/reset-password", async (req, res) => {
    ...
    try {
      ...
      await storage.updateStudent(req.params.reg_no, { password: hashedPassword });
  ...
  app.post("/api/admin/upload-students-csv", async (req, res) => {
    ...
    try {
      ...
      for (const row of rows) {
        if (row.trim()) {
          const [reg_no, name, department, password] = row.split(',').map((field: any) => field.trim().replace(/"/g, ''));
  ...
  app.post("/api/admin/upload-progress-csv", async (req, res) => {
    ...
    try {
      ...
      for (const row of rows) {
        if (row.trim()) {
          const [reg_no, problem_id, status] = row.split(',').map((field: any) => field.trim().replace(/"/g, ''));
  ...
  app.delete("/api/student/:reg_no/goals/:goal_id", async (req, res) => {
    if (!req.isAuthenticated()) {
      return res.status(401).json({ message: "Not authenticated" });
    }
    ...
    try {
      ...
      const success = await storage.deleteStudentGoal(req.params.reg_no, goalId);
      
      if (success) {
        res.json({ message: "Goal deleted successfully" });
      } else {
        res.status(404).json({ error: "Goal not found or access denied" });
      }
    } catch (error) {
